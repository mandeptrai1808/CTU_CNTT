...
void update(canh a[][size], int n, float *GNNTT, float TGT, canh x[], canh PA[]){
		...
		if(j!=dau && !a[dau][j].da_dung && !check_chu_trinh(x, i, j)){
			...
			*CD = can_duoi(a, n,i+1, *TGT);
			if(*CD < *GNNTT){
				...
				if (i == n-2) update(a, n, GNNTT, *TGT, x, PA);
...
void in_PA(canh PA[], int n){
	...
	printf("CHU TRINH: \n");
	for(int i = 0 ; i < n-1; i++) printf("%c -> ", PA[i].dau +97);
	printf("%c\n", PA[0].dau + 97);
	printf("Tong chieu dai: %.2f", sum);
}
...
int main(){
	...
	canh x[size], PA[size];
	nhanh_can(a, n, 0, 2, &TGT, &CD, &GNNTT, x, PA);
	in_PA(PA, n);
