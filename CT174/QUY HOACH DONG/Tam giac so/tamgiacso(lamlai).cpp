...
int FindMaxLastCol(int F[], int n){
	int idMax = 0;
	int Fmax = F[0];
	for(int i = 0; i < n; i++){
		if (F[i] > Fmax){
...
void TaoBang(int a[][size], int F[][size], int n){
	F[0][0] = a[0][0];
	F[1][0] = a[1][0] + F[0][0];
	F[1][1] = a[1][1] + F[0][0];
...
void TraBang(int F[][size], int a[][size], int PA[], int n){
	int j = FindMaxLastCol(F[n-1], n);
	PA[n-1] = a[n-1][j];
	for(int i = n-1; i >=0; i--){
		j = CS_Min(F, i ,j);
		PA[i-1] = a[i-1][j];
...
void in_KQ(int PA[], int n){
	...
	printf("%d\n", PA[n-1]);
	printf("Tong: %d", sum + PA[n-1]);
}

int main(){
	int a[size][size], F[size][size], PA[size];
	int n;
